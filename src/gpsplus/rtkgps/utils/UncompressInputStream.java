...
public class UncompressInputStream extends FilterInputStream
{
	...
	main_loop: do
	{
		...
		if (l_free_ent > l_maxcode)
		{
		    ...
		    if (DBG)
		        Log.d(TAG,"Code-width expanded to " + l_n_bits);
		...
		if (code == TBL_CLEAR && block_mode)
		{
		    ...
		    if (DBG)  Log.d(TAG,"Code tables reset");
	...
	if (DBG)
	{
	    Log.d(TAG,"block mode: " + block_mode);
	    Log.d(TAG,"max bits:   " + maxbits);
	}
    ...
    static final String TAG = UncompressInputStream.class.getSimpleName();
}